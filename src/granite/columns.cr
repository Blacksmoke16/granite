...
module Granite::Columns
  module ClassMethods
    ...
    private struct ColumnInfo(T) < ColumnBase
      ...
      getter name : String
      getter type : T.class
      getter default : T? = nil
      getter auto : Bool = false
      getter primary : Bool = false

      def initialize(@name : String, @nilable : Bool, @auto : Bool = false, @primary : Bool = false, @default : T? = nil, @type : T.class = T)
      end
    end
